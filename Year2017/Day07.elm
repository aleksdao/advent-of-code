module Year2017.Day07 exposing (..)
...
findBottom : Input -> String
findBottom input =
    let
        combos =
            List.filter (Tuple.second >> (\list -> not (List.isEmpty list))) input

        possibleBases =
            List.map Tuple.first combos
...
isBaseAChild : String -> List String -> Bool
isBaseAChild base children =
    List.member base children
